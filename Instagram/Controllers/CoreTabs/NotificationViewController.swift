    ...
    private func fetchNotifications() {
        for x in 0...100 {
            let post = UserPost(identifier: "",
                                ...
                                createdDate: Date(),
                                taggedUsers: [])
            let model = UserNotification(type: x % 2 == 0 ? .like(post: post) : .follow(state: .not_following), text: "hello world", user: User(username: "joe", bio: "", name: (first:"",last: ""), profilePhote: URL(string: "https://www.google.com")!, birthDate: Date(), gender: .male, count: UserCount(followers: 1, following: 1, posts: 1), joinDate: Date()))
